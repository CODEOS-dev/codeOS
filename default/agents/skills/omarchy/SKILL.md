...
description: >
  ...
  settings, display config, and user-facing omarchy commands. Excludes codeOS
  source development through `omarchy dev link` workflows.
---

# codeOS Skill

Manage [codeOS](https://codeos.org/) Linux systems - a beautiful, fun, agentic Arch Linux distribution with Hyprland.

This skill is for end-user customization on installed systems.
It is not for contributing to codeOS source code.
...
**Do NOT use this skill for codeOS development tasks** (editing the codeOS source tree, creating migrations, or running `omarchy dev ...` workflows).
...
- [`plugins.md`](plugins.md) - the codeOS shell: bar layout, widgets, plugins, idle behavior
...
- [`contributing.md`](contributing.md) - reporting codeOS bugs and submitting fixes upstream
...
├── shell/                  # codeOS shell source and defaults
...
If the request is to develop codeOS itself, this skill is out of scope. Follow repository development instructions instead of this skill.

## Privilege Escalation

For an interactive script or command run in a visible terminal, use `sudo` for
privileged work. codeOS may grant passwordless `sudo` access to particular
...
| **codeOS shell** | Status bar + notifications (Quickshell) | `~/.config/omarchy/shell.json` |
| **Launcher/menus** | Quickshell menu | `~/.config/omarchy/extensions/omarchy-menu.jsonc` |
| **Alacritty/Foot/Kitty/Ghostty** | Terminals | `~/.config/<terminal>/` |
| **codeOS OSD** | On-screen display | Quickshell plugin |

## Command Discovery

codeOS ships a single `omarchy` CLI that dispatches to all `omarchy-*` binaries via `omarchy <group> <action>`. Always prefer this form — it is self-documenting and stable. The underlying `omarchy-*` binaries still exist on `PATH` and remain safe to read for source.
...
The codeOS shell (bar, notifications, plugins, idle) is configured in
...
# - codeOS shell: shell.json and user plugin code under ~/.config/omarchy/plugins/ hot-reload on save
...
omarchy version                 # Show codeOS version
...
This skill intentionally does not cover codeOS source development. Do not use this skill for:
...
- "Report this bug to codeOS" -> Gather diagnostics and a capture of the problem, then file it (see `contributing.md`)
