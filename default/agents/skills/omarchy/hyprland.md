...
codeOS configures Hyprland in Lua. User files are loaded after codeOS's
defaults, so overrides go here:

```
~/.config/hypr/
├── hyprland.lua       # Main config (loads codeOS defaults, then user files)
...
Window rules go in `~/.config/hypr/hyprland.lua` or a required Lua module. Prefer codeOS's `o.window(match, rules)` helper — see examples in `$OMARCHY_PATH/default/hypr/windows.lua`.
