...
const navLinks = [
  ...
  { href: 'https://plugins.codeOS.org', label: t('Plugins') },
          ...
          <div className="ml-auto flex items-center gap-2.5">
            <div className="hidden items-center gap-1 sm:flex">
              <TooltipProvider delay={300}>
                <NavTooltip label={t('Search codeOS')} shortcut="⌘K / Ctrl+K">
                  {search}
                </NavTooltip>
                ...
                <NavTooltip label={t('View codeOS on GitHub')}>
                  <Button
                    variant="ghost"
                    size="icon"
                    aria-label={t('codeOS on GitHub')}
                    data-nav-glyph
                    className="relative h-8 w-8 text-text-secondary transition-[background-color,transform] hover:text-text before:absolute before:-inset-1 lg:h-[calc(var(--pxr)*3)] lg:w-[calc(var(--pxr)*3)]"
                    nativeButton={false}
                    render={<a href="https://github.com/omacom/codeOS" />}
                  >
