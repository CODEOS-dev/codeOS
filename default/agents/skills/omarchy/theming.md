...
are codeOS's own themes. From a theme cloned by `omarchy theme install`, codeOS
...
codeOS tells a cloned theme from the user's own by the `.git` directory a clone
leaves behind.

To change how codeOS themes an app for every theme, write the template rather
than the theme: `~/.config/omarchy/themed/<config-name>.tpl` overrides the
built-in one. See `docs/theming.md` in the codeOS repo.
